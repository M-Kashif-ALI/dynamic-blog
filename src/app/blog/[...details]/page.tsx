...
const Page = ({ params }: PageProps) => {
  ...
  const postId = Number(details[0]);
  const post = latestPosts.find((item) => item.id === postId);

  if (!post) {
    return (
      <div className="bg-[#242535] text-white md:py-10 px-2">
        <h1 className="text-4xl text-center font-bold">Post Not Found</h1>
      </div>
    );
  }
    ...
    <div className="bg-[#242535] md:px-10 text-white md:py-10 px-2 overflow-y-hidden">
      <div className="flex flex-col items-center justify-center gap-4">
        ...
        <Image
          ...
          width={400}
          height={400}
          className="md:h-[60vh] md:w-[50%] rounded-md border-[2px] border-gray-400 w-[90%]"
        />
